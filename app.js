...
const updateAutoProgress = ({ visible, value, max, text }) => {
  if (!autoProgress) {
    return;
  }
  ...
  if (typeof max === "number") {
    autoProgressBar.max = max;
  }

  if (typeof value === "number") {
    autoProgressBar.value = value;
    const percent = autoProgressBar.max
      ? Math.round((autoProgressBar.value / autoProgressBar.max) * 100)
      : 0;
    autoProgressValue.textContent = `${percent}%`;
  }
...
const generateOutputs = async () => {
  ...
  updateAutoProgress({ visible: false, value: 0, max: 100 });
  clearOutputs();

  try {
    const rawLines = moduleInput.value.split("\n");
    const hasManualInput = rawLines.some((line) => line.trim());
    const shouldAutoSplit = !hasManualInput && !detectedModules;
    ...
    if (shouldAutoSplit) {
      updateAutoProgress({
        ...
        value: 0,
        max: 100,
        text: "Autogenerating module ranges...",
      });
    }

    let modules = hasManualInput
    ...
    if (shouldAutoSplit) {
      updateAutoProgress({
        ...
        value: 0,
        text: `Autogenerated ${modules.length} module ranges.`,
      });
    }
    ...
    for (const module of modules) {
      ...
      if (shouldAutoSplit) {
        ...
        updateAutoProgress({
          value: completed,
          text: `Creating module ${completed} of ${modules.length}...`,
        });
      }
    }
